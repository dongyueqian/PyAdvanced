...
def get_fundcode():
    ...
    fundcode.to_csv('./案例/基金代码爬取/fundcode.csv', index=False)
...
def main(fundcode):
    ...
    lsjz = info['lsjz']
    lsjz.to_csv('./基金代码爬取/%s_lsjz.csv' % fundcode, index=False)  # 将基金历史净值以csv格式储存
    page = 1
    while page < 2:
        page += 1
        print(lsjz)
        ...
        if info is None:
            break
        lsjz = info['lsjz']
        lsjz.to_csv('./基金代码爬取/%s_lsjz.csv' % fundcode, mode='a', index=False, header=False)  # 追加存储
...
if __name__=='__main__':
    ...
    get_fundcode()
    fundcode = '519961'
    fundcodes = pd.read_csv('./基金代码爬取/fundcode.csv', converters={'fundcode': str})
    # 获取所有基金净值数据
